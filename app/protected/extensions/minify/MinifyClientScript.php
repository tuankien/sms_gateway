...
class MinifyClientScript extends CClientScript
{
	...
	private function combineAndCompress($type, $urls, $pos)
	{
		...
		if ($this->_renewFile)
		{
			...
			foreach ($urls as $key => $file)
                        {
                                // Tungnv: thay doi url tuong doi sang url tuyet doi
                                $buffer = file_get_contents($this->basePath.DIRECTORY_SEPARATOR.$file);
                                if($buffer)
                                {
                                    if($type == 'css')
                                    {
                                        ...
                                        $regex = "/url\s*\(\s*['\"]?([^'\"]\S*[^'\"])['\"]?\s*\)\s*(\S*)/";
                                        $buffer = preg_replace($regex,"url(".$fileUrl."/$1) $2",$buffer);

                                        // @import
                                        // $regex = "/@import[\s]+['\"]{1}([^'\"][\S]*[^'\"])['\"]{1}(.*)/";
                                        // $buffer = preg_replace($regex,"@import \"".$this->basePath.$fileUrl."/$1\"$2",$buffer);
                                    }

                                    if ($type == 'js' && $this->compressJs && strpos($file, '.min.') === false && strpos($file, '.pack.') === false)
                                    {
                                        $buffer = $this->minifyJs($buffer);
                                    }
                                    elseif ($type == 'css' && $this->compressCss && strpos($file, '.min.') === false && strpos($file, '.pack.') === false)
                                    {
	...
	{
		Yii::import('application.extensions.minify.jsmin.*');
	...
	{
		Yii::import('application.extensions.minify.csstidy.*');
